...
void init() {
	...
	cout << "Keyboard Input : up, down, left, right with modifier key(shift) for turning, moving" << endl;
	cout << endl;
...
// with ctrl key, e에 a기준으로 돌린걸 곱함, 그니까 eye position이 바뀜
void turnLeft_2() {
	mat4 M(1.0);
	M = glm::translate(M, C.a);
	M = glm::rotate(M, radians(-10.0f), C.u);//u 기준으로 10도씩 회전
	M = glm::translate(M, -C.a);

	C.e = M * vec4(C.e, 1.0f); //homogeneous coordinate

	//M = glm::rotate(M, radians(10.0f), C.u);
	//C.f = normalize(M * vec4(C.f, 1.0f));
	C.f = normalize(C.a - C.e);
	C.r = normalize(cross(C.f, C.u));
	C.u = normalize(cross(C.r, C.f));
}
...
void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	if ((action == GLFW_PRESS || action == GLFW_REPEAT) && (mods & GLFW_MOD_CONTROL))
	{
		...
		{
			//navigation
		case GLFW_KEY_LEFT:		cout << "left turn" << endl;	turnLeft_2();	break;
		case GLFW_KEY_RIGHT:	cout << "right turn" << endl;	turnRight_2();	break;
		case GLFW_KEY_UP:		cout << "move up" << endl;		moveUp();		break;
		case GLFW_KEY_DOWN:		cout << "move down" << endl;	moveDown();		break;
		}
	}

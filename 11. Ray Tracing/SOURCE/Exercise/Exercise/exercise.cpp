...
#include <glm/gtx/string_cast.hpp> // gim::to_string()
...
int m = 0, m_prev = -1; // Width of the image Mm = windowW
...
vec3 intensity(const Ray& ray, const Light l[], int nLights, int depth, int E = -1)
{
	...
	for (int i = 0; i < nLights; i++) {
		...
		if (jObject == 7) // Not shadowed -> ±×´Ï±î ¸¶Áö¸·±îÁö ¾ÈºÎ´ÚÄ¡°í °¡Àå Å« °ÑÀÇ ±¸¿¡ ´ê´Â °Å
		{
			// Phong reflection
	...
	{
		// Reflection ray
		vec3 r = normalize(reflect(ray.p0 - ray.p1, n));	// Direction to the viewer
...
void rayTracing()
{
	...
	// world -> eye coordinate direction
	for (int i = 0; i < nLights; i++)
	...
	// Perspective projectioni for ray tracing
	float fovy = 27.0;
	float dn = nearDist;
	float df = farDist;
...
#pragma omp parallel for schedule(dynamic) if (useOpenMP)
	// static, quide, auto, runtime -> ±úÁüÇö»ó ¹ß»ý! ·£´ý? thread 16
	//OpenMP Áö¿ø ²ô´Ï±î ±úÁü ´úÇÔ -> ¿Ö?? thread 1
	for (int i = 0; i < m; i++)
		for (int j = 0; j < n; j++)
		{
			...
			vec3 s(x_i, y_j, -dn);  // Start point
			vec3 e = (df / dn) * s; // End point
...
void init()
{
	...
	center_world[nSpheres] = vec3(0, 0, -r); radius[nSpheres] = 0.5f; nSpheres++;
	center_world[nSpheres] = vec3(0, 0, 0); radius[nSpheres] = 10.0f; nSpheres++;//enclosing sphere

...
void	keyboard(GLFWwindow* windwo, int key, int code, int action, int mods);
...
//endpoints of the three line segments to convert triangle
float	point[3][2] = { {0,0},{0,0},{0,0} };//
float	line_point[2][2] = { {0,0},{0,0} };//
...
void mouseButton(GLFWwindow* window, int button, int action, int mods) {
	...
	if (action == GLFW_RELEASE && button == GLFW_MOUSE_BUTTON_LEFT) {
		if (point_num < 3)
		{
			inputMode = InputMode::CLICKED;
			line_point[0][0] = xw; line_point[0][1] = yw;//
			line_point[1][0] = xw; line_point[1][1] = yw;//

			point[point_num][0] = line_point[0][0];//
			point[point_num][1] = line_point[0][1];//
			point_num += 1;

			if (point_num == 3) inputMode = InputMode::COMPLETE;//
		}

	}
	else if (action == GLFW_PRESS && button == GLFW_MOUSE_BUTTON_LEFT) {
		if (point_num == 3) point_num = 0;
	}
}
//
void mouseMove(GLFWwindow* window, double x, double y) {
	if (inputMode == InputMode::CLICKED) {
		screen2world((float)x, (float)y, line_point[1][0], line_point[1][1]);
	}
}
...
void render(GLFWwindow* window) {
	...
	if (inputMode > InputMode::NONE) {
		if (point_num == 1) {
			...
			glBegin(GL_LINES);
			glVertex2f(line_point[0][0], line_point[0][1]);
			glVertex2f(line_point[1][0], line_point[1][1]);
